...
export default function SignInScreen() {
  ...
  const isLoading = isGoogleClick || isAppleClick || isGitHubClick;
    ...
    <SafeAreaView className='flex-1 bg-primary dark:bg-secondary' edges={['top']}>
      ...
      <View className='px-6 pt-4'>
        ...
        <View className='mt-6 rounded-[30px] border border-white/20 bg-white/10 p-3' >
          <Image source={require('@/assets/images/auth.png')} style={{ width: '100%', height: 300 }} />
        </View>
      </View>
      <View className='mt-8 flex-1 rounded-t-[36px] bg-card px-6 pb-8 py-6'>
        <View className="self-center rounded-full bg-secondary px-3 py-1">
          ...
          <View className='mt-6'>
            <Pressable className={`mb-3 h-14 flex-row items-center rounded-2xl border border-border bg-card px-4 active:opacity-90 ${isLoading ? "opacity-70" : ""
              }`}
              disabled={isLoading}
              onPress={() => handleSocialAuth("oauth_google")}>
              <View className='h-8 w-8 items-center justify-center rounded-full bg-white'>
                <Image source={require('@/assets/images/google.png')} style={{ width: 18, height: 18 }} />
              </View>
